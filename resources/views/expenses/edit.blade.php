                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('expenses.update', $expense->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $expense->amount) }}" step="0.01" required>
                            @error('amount')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('purchase_from') is-invalid @enderror" id="purchase_from" name="purchase_from">
                                ...
                                <option value="Supershop" {{ old('purchase_from', $expense->purchase_from) == 'Supershop' ? 'selected' : '' }}>Supershop</option>
                                <option value="eCommerce" {{ old('purchase_from', $expense->purchase_from) == 'eCommerce' ? 'selected' : '' }}>eCommerce</option>
                                <option value="Bazar" {{ old('purchase_from', $expense->purchase_from) == 'Bazar' ? 'selected' : '' }}>Bazar</option>
                            </select>
                            @error('purchase_from')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('assigned_to_user_id') is-invalid @enderror" id="assigned_to_user_id" name="assigned_to_user_id" required>
                                ...
                                @foreach ($users as $userOption)
                                    <option value="{{ $userOption->id }}" {{ old('assigned_to_user_id', $expense->assigned_to_user_id) == $userOption->id ? 'selected' : '' }}>{{ $userOption->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            @if ($expense->receipt_path)
                                <div class="mt-2">
                                    Current Receipt:<br>
                                    <img src="{{ asset('storage/' . $expense->receipt_path) }}" alt="Receipt" class="img-fluid" style="max-width: 200px;">
                                </div>
                            @endif
                        </div>
